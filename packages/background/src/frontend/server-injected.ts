...
import {
	...
	SOLANA_RPC_METHOD_PREPARE_OTA_TX,
...
async function handle<T = any>(
	...
	switch (method) {
		...
		case SOLANA_RPC_METHOD_PREPARE_OTA_TX:
...
async function handleConnect(
	...
	if (await ctx.backend.isApprovedOrigin(origin)) {
		...
		logger.debug("show response on approve:", resp)
	...
	if (didApprove) {
		// const publicKey = ota
		// 	? await ctx.backend.otaPublickey()
		// 	: await ctx.backend.activeWallet();
...
async function handleSignAndSendTx(
	...
	try {
		...
		if (didApprove) {
			const otaPubkey = await ctx.backend.otaPublickey()
			let signer = walletAddress
			if (walletAddress === otaPubkey)
				signer = await ctx.backend.delegateWallet()
			const sig = await ctx.backend.signAndSendTx(transaction, signer, options)
			resp = [sig]
		}
	} catch (err) {
...
async function handleSignTx(
	...
	let resp: RpcResponse<[string, string]>
	const { didApprove, transaction } = uiResp.result
	...
	try {
		if (didApprove) {
			...
			const sig = await ctx.backend.signTransaction(transaction, signer)
			resp = [signer, sig]
		}
	} catch (err) {
...
async function handlePrepareOtaTx(
	...
	const resp = await ctx.backend.prepareOtaTransaction(transaction, isVersioned)
	return [resp]
}
