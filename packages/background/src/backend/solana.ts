...
export const LOAD_SPL_TOKENS_REFRESH_INTERVAL = 30 * 1000;
export const RECENT_BLOCKHASH_REFRESH_INTERVAL = 30 * 1000;
...
export class SolanaConnectionBackend {
	...
	private connection?: Connection = new Connection(RPC_DEVNET_ENDPOINT);
	private url?: string;
			...
			setInterval(async () => {
				...
				this.events.emit(BACKEND_EVENT, {
					...
					data: {
						...
						customSplTokenAccounts:
							SolanaSplConnection.customSplTokenAccountsToJson(data),
					},
				});
	...
	> {
		const resp =
			await this.connection!.getLatestBlockhashAndContext(commitment);
		return resp;
	}
