...
import {
	...
	KeyringStore,
	KeyringStoreState,
} from "../keyring/keystore"
import { SolanaConnectionBackend } from "./solana"
import EventEmitter from "eventemitter3"
import { DerivationPath, WalletPublicKey } from "../keyring"
import { ValetDB, UserData } from "../database"
...
export class Backend {
	...
	async signAllTransactions(
		...
		return await Promise.all(
			txMessages.map((t) => this.signTransaction(t, walletAddress))
		)
	}
	...
	async otaPublickey(): Promise<string> {
		return await this.valetDb.publicKey()
	}

	// returns a prepared ota transaction, and delegate
	async prepareOtaTransaction(
		tx: string,
		isVersioned: boolean
	): Promise<[string, string]> {
		return await this.valetDb.prepareTransaction(tx, isVersioned)
	}
	...
	async recentBlockhash(commitment?: Commitment): Promise<string> {
		const { blockhash } =
			await this.solanaConnectionBackend.getLatestBlockhash(commitment)
		return blockhash
	}
	...
	async keyringStoreCreate(
		...
		const delegateWallet = await this.delegateWallet()
		const commitment = await this.solanaCommitmentRead()
	...
	async keyringStoreUnlock(password: string): Promise<String> {
		...
		await this.valetDb.unlock(password)
	...
	async keyringStoreLock() {
		this.keyringStore.lock()
		await this.valetDb.lock()
		this.events.emit(BACKEND_EVENT, {
	...
	async keyringStoreReadAllPubkeys(): Promise<WalletPublicKey> {
		const pubkeys = this.keyringStore.publicKeys()
		const otaPubkey = await this.otaPublickey()
	...
	async delegateWallet(): Promise<string> {
		return await this.keyringStore.delegateWallet()
	}
	...
	async delegateWalletUpdate(newWallet: string): Promise<string> {
		await this.keyringStore.delegateWalletUpdate(newWallet)
		this.events.emit(BACKEND_EVENT, {
			name: NOTIFICATION_DELEGATE_WALLET_UPDATED,
			data: {
				delegateWallet: newWallet,
			},
		})
		return SUCCESS_RESPONSE
	}
	...
	async signIn(): Promise<string> {
		await this.valetDb.signIn()
		return SUCCESS_RESPONSE
	}

	async signOut(): Promise<string> {
		await this.valetDb.signOut()
		this.keyringStore.lock()
		this.events.emit(BACKEND_EVENT, {
			name: NOTIFICATION_SIGNOUT_SUCCESSFUL,
		})
		return SUCCESS_RESPONSE
	}

	async getUserData(): Promise<UserData> {
		return this.valetDb.getUserData()
	}

	async isMnemonicInDb(): Promise<boolean> {
		return this.valetDb.isMnemomicInDb
	}

	async keyringStoreCreateNew(
		mnemonic: string,
		password: string
	): Promise<string> {
		PasswordManager.setPassword(password)
		await this.valetDb.setMnemonic(mnemonic)
		...
		const url = await this.solanaConnectionUrl()
		const activeWallet = await this.activeWallet()
		const delegateWallet = await this.activeWallet()
		const commitment = await this.solanaCommitmentRead()
	...
	async keyringStoreCreateExisting(password: string): Promise<string> {
		const mnemonic = await this.valetDb.getMnemonic(password)
		const derivationPath: DerivationPath = "bip44-change" // todo: make dynamic

		PasswordManager.setPassword(password)

		await this.keyringStore.init(mnemonic, derivationPath, password)
		await this.valetDb.persist()
	...
	async isIdTokenValid(): Promise<boolean> {
		return await this.valetDb.isIdTokenValid()
	}

	async otaTransferDelegateServer(
		from: string,
		to: string,
		password: string
	): Promise<string> {
		const [delegate, transaction] = await this.valetDb.transferDelegateServer(
			from,
			to,
			password
		)
		...
		const signature = await this.signTransaction(transaction, delegate)
		tx.addSignature(new PublicKey(delegate), Buffer.from(bs58.decode(signature)))
		const commitment = await this.solanaCommitmentRead()
		return await this.solanaConnectionBackend.sendRawTransaction(
			tx.serialize(),
			{
	...
	async otaTransferDelegateLocal(from: string, to: string): Promise<string> {
		const transaction = await this.valetDb.transferDelegateLocal(from, to)
		const tx = deserializeTransaction(transaction)
		const toSignature = await this.signTransaction(transaction, to)
		tx.addSignature(new PublicKey(to), Buffer.from(bs58.decode(toSignature)))
		const fromSigature = await this.signTransaction(transaction, from)
		tx.addSignature(new PublicKey(from), Buffer.from(bs58.decode(fromSigature)))
		const commitment = await this.solanaCommitmentRead()
		return await this.solanaConnectionBackend.sendRawTransaction(
			tx.serialize(),
			{
