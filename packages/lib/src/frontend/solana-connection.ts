import { SolanaConnectionBackend } from "../backend/solana";
...
import { SerializedTokenAccountsFilter, SolanaTokenAccountWithKeyAndProgramIdString, TokenMetadataString, deserializeTokenAccountsFilter } from "../token/types";
import { SolanaSplConnection } from "../token";
...
export function start(
	...
	const solanaConnectionInjected = (() => {
		...
		s.handler(withContextPort(b, events, handleInjected));
		return s;
	})();
...
async function handleCustomSplTokenAccounts(
	...
	return [SolanaSplConnection.customSplTokenAccountsToJson(resp)];
}
