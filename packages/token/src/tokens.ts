...
export async function customSplTokenAccounts(
	...
	const tokenProgram22Interface = tokenInterface.withProgramId(
	...
	const [accountInfo, tokenAccounts, token2022Accounts] = await Promise.all([
		...
		//
		provider.connection.getAccountInfo(new PublicKey(publicKey)),
		//
		// Fetch tokens.
		//
		fetchTokens(publicKey, tokenProgramInterface),
		fetchTokens(publicKey, tokenProgram22Interface),
	])
