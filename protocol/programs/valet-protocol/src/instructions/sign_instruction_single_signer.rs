...
pub fn sign_instruction_single_signer(ctx: Context<SignInstructionSingleSigner>, instruction_data: Vec<u8>,) -> Result<()> {
    ...
    let ix = Instruction {
        accounts:ctx.remaining_accounts.iter()
            .map(|account| {
                ...
                AccountMeta {
                    is_signer: if account.key() == wallet_signer_key { true } else { account.is_signer },
