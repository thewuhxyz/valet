...
router.post("/sign-transaction", async (req, res) => {
	try {
		...
		let { success: dappPayloadParseSuccess } =
			dappSignTransactionDataSchema.safeParse(dappPayload)
		...
		const { provider, providerToken, transaction } =
			dappSignTransactionDataSchema.parse(dappPayload)
		...
		let providerInfo = await getGoogleToken(providerToken, dappClientId)
		...
		const signedTransaction = await otaServerClient.signTransaction(
			userDelegate,
			transaction
		)

		res.json(
			OtaResponse.signTransaction({
