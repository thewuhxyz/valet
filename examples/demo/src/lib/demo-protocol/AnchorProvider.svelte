<script lang="ts">
	...
	import { DemoProgramClient } from "./client"
	import { demoProgram } from "$lib/stores"

	export let network: string,
		config: Commitment | ConnectionConfig | undefined = "singleGossip"
	...
	function defineProgramAndProvider(walletStore: WalletStore): {
		...
		try {
			...
			const demoProgramClient = new DemoProgramClient(anchorProvider)

			provider.set(anchorProvider)
			demoProgram.set(demoProgramClient)
